...
import ListItem from '../components/AttendanceHistoryItem';
import { getRegularizationHistory } from '../services/AttendanceHistoryService';
...
class AttendanceHistoryScreen extends Component {
  ...
  getHistory=async () => {
    const response = await getRegularizationHistory(this.props.user);
    console.log('attHistory', response);
    if (response) {
      this.historyData = response.AttendanceHistory;
